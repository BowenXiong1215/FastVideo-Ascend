...
def profile_environment(args: argparse.Namespace) -> dict[str, str | None]:
    ...
    return {
        "FASTVIDEO_ATTENTION_BACKEND": ("VIDEO_SPARSE_ATTN_H3" if use_vsa else getattr(
            args, "dense_attention_backend", "FLASH_ATTN")),
...
def build_generator_config(args: argparse.Namespace) -> GeneratorConfig:
    ...
    experimental: dict[str, object] = {
        "attention_backend": ("VIDEO_SPARSE_ATTN_H3" if use_vsa else getattr(
            args, "dense_attention_backend", "FLASH_ATTN")),
