...
accelerator RNG snapshots need a device runner; custom generator restore is
...
def test_rng_snapshot_restores_training_generator(tmp_path: Path) -> None:
    class _Method:
        cuda_generator = torch.Generator(device="cpu").manual_seed(123)

    checkpoint_dir = _make_checkpoint_dir(tmp_path, 1)
    manager = _make_manager(tmp_path)
    manager.method = _Method()
    manager._save_rng_snapshot(checkpoint_dir)
    expected = manager.method.cuda_generator.get_state().clone()
    manager.method.cuda_generator.manual_seed(999)

    manager.load_rng_snapshot(str(checkpoint_dir))

    assert torch.equal(manager.method.cuda_generator.get_state(), expected)
