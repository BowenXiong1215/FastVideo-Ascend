...
class CheckpointManager:
    ...
    def _save_rng_snapshot(self, checkpoint_dir: Path) -> None:
        ...
        Each rank saves its own file because accelerator RNG and
        ...
        if current_platform.is_cuda_alike() or current_platform.is_npu():
            accelerator = current_platform.get_torch_device()
            rng["accelerator_rng"] = accelerator.get_rng_state()
            rng["accelerator_type"] = current_platform.device_type
        rng["generator_rng"] = self.method.cuda_generator.get_state()
        torch.save(
            rng,
    ...
    def load_rng_snapshot(
        ...
        accelerator_rng = rng.get("accelerator_rng", rng.get("cuda_rng"))
        if accelerator_rng is not None and (current_platform.is_cuda_alike()
                                            or current_platform.is_npu()):
            current_platform.get_torch_device().set_rng_state(accelerator_rng)
        generator_rng = rng.get("generator_rng", rng.get("gen_cuda"))
        if generator_rng is not None:
            self.method.cuda_generator.set_state(generator_rng)
        logger.info(
