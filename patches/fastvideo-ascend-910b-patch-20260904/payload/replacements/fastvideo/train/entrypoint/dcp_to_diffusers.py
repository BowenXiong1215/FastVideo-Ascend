...
def _persist_runtime_component_config(*, module: Any, module_dir: Any) -> None:
    """Persist FastVideo runtime settings that are not HF architecture fields.

    The MiniMax-H3 training wrapper intentionally makes every student parameter
    BF16 so FSDP can flatten each parameter group.  The released base checkpoint,
    however, defaults to mixed FP32/BF16 inference.  Because an export starts by
    copying the base directory, omitting this field makes a trained BF16 student
    get reconstructed with mixed parameter dtypes and fail during FSDP lazy init.
    """
    from pathlib import Path

    config = getattr(module, "config", None)
    uniform_parameter_dtype = getattr(config, "uniform_parameter_dtype", None)
    if uniform_parameter_dtype is None:
        return

    config_path = Path(module_dir) / "config.json"
    if not config_path.is_file():
        raise FileNotFoundError(f"Cannot persist runtime dtype contract; missing {config_path}")

    data = json.loads(config_path.read_text(encoding="utf-8"))
    data["uniform_parameter_dtype"] = bool(uniform_parameter_dtype)
    ...
    logger.info(
        "Persisted %s uniform_parameter_dtype=%s",
        config_path,
        data["uniform_parameter_dtype"],
    )
    ...
    *,
    ...
    for module_name in module_names:
        ...
        if _rank() == 0:
            ...
            _persist_runtime_component_config(
