...
def iter_hf_to_custom_state_dict(
    ...
    """Map checkpoint tensors lazily, retaining only incomplete merge groups.

    Large FSDP checkpoints must be placed one tensor at a time.  Converting the
    input iterator to a dictionary first keeps the complete, unsharded model
    resident until placement starts and can exhaust an accelerator before FSDP
    has a chance to retain only its local shard.
    """
    to_merge_params: defaultdict[str, dict[int, torch.Tensor]] = defaultdict(dict)
    if isinstance(hf_param_sd, dict):
        hf_param_sd = iter(hf_param_sd.items())
    ...
    for source_param_name, full_tensor in hf_param_sd:
        ...
        if merge_index is not None:
            if num_params_to_merge is None:
                raise ValueError(f"Missing merge count for checkpoint parameter {source_param_name}")
            to_merge_params[target_param_name][merge_index] = full_tensor
            if len(to_merge_params[target_param_name]) != num_params_to_merge:
                continue
            full_tensor = torch.cat(
                [to_merge_params[target_param_name][i] for i in range(num_params_to_merge)],
                dim=0,
            )
            del to_merge_params[target_param_name]
    ...
    if to_merge_params:
        incomplete = ", ".join(sorted(to_merge_params))
        raise ValueError(f"Checkpoint ended with incomplete merged parameters: {incomplete}")
