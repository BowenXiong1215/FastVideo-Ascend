...
def maybe_load_fsdp_model(
    ...
    # An NPU cannot stage the complete unsharded H3 checkpoint: its BF16 size is
    # close to a 910B's capacity even though the final FSDP shard is small.
    # Read on the host and move/distribute one tensor at a time. This is only a
    # loading policy; parameters still reside on NPU unless cpu_offload=True.
    stage_weights_on_cpu = cpu_offload or current_platform.is_npu()
    weight_iterator = safetensors_weights_iterator(weight_dir_list, to_cpu=stage_weights_on_cpu)
    logger.info(
        "Loading transformer weights with CPU staging=%s, FSDP CPU offload=%s",
        stage_weights_on_cpu,
        cpu_offload,
    )
...
def load_model_from_full_model_state_dict(
    ...
    mapped_tensors = iter_hf_to_custom_state_dict(
        full_sd_iterator,
        param_names_mapping,  # type: ignore[arg-type]
        reverse_param_names_mapping,
    )
    # Place each mapped tensor immediately. The source iterator can then close
    # old safetensors shards instead of retaining a full unsharded checkpoint.
    for target_param_name, full_tensor in mapped_tensors:
