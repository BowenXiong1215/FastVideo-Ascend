...
class MiniMaxH3Qwen3VLTextAttention(nn.Module):
    ...
    def __init__(self, config: MiniMaxH3Qwen3VLConfig, prefix: str) -> None:
        ...
        # TextEncoderLoader constructs this model inside the requested dtype
        # context (BF16 for H3).  RMSNorm otherwise defaults its parameter to
        # FP32 on NPU, mixing FP32 norms with BF16 linears inside one FSDP
        # parameter group and failing FSDP2's lazy-init dtype assertion.
        # RMSNorm.forward_native still performs variance math in FP32.
        parameter_dtype = torch.get_default_dtype()
        self.q_norm = RMSNorm(self.head_dim, eps=config.rms_norm_eps, dtype=parameter_dtype)
        self.k_norm = RMSNorm(self.head_dim, eps=config.rms_norm_eps, dtype=parameter_dtype)
...
class MiniMaxH3Qwen3VLTextDecoderLayer(nn.Module):
    ...
    def __init__(self, config: MiniMaxH3Qwen3VLConfig, prefix: str) -> None:
        ...
        parameter_dtype = torch.get_default_dtype()
        self.input_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps, dtype=parameter_dtype)
        self.post_attention_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps, dtype=parameter_dtype)
