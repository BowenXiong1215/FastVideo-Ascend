...
def _apply_h3_checkpoint_arch_configs(model_path: str, fastvideo_args: FastVideoArgs,
    ...
    if (transformer_dir / "config.json").is_file():
        transformer_config = get_diffusers_config(str(transformer_dir))
        fastvideo_args.pipeline_config.dit_config.update_model_arch(transformer_config)
        if "uniform_parameter_dtype" in transformer_config:
            uniform_parameter_dtype = transformer_config["uniform_parameter_dtype"]
            if not isinstance(uniform_parameter_dtype, bool):
                raise TypeError(
                    "MiniMax-H3 transformer config field "
                    "uniform_parameter_dtype must be a JSON boolean, got "
                    f"{type(uniform_parameter_dtype).__name__}."
                )
            fastvideo_args.pipeline_config.dit_config.uniform_parameter_dtype = uniform_parameter_dtype
            logger.info(
                "MiniMax-H3 parameter dtype contract from config: "
                "uniform_parameter_dtype=%s",
